 ...
 * Build: http://modernizr.com/download/#-fontface-flexbox-opacity-rgba-cssanimations-generatedcontent-csstransforms-csstransforms3d-csstransitions-inlinesvg-svg-touch-shiv-cssclasses-prefixed-teststyles-testprop-testallprops-prefixes-domprefixes-css_boxsizing-css_remunit-css_vhunit-css_vmaxunit-css_vminunit-css_vwunit-unicode-url_data_uri-load
 */

function setupMaps() {
	if (mapbox.length) {
		map = L.mapbox.map("map");
		map.addLayer(L.mapbox.tileLayer("mixd.map-j47aonim", {
			detectRetina: !0,
			retinaVersion: "mixd.map-e91svu3q"
		}));
		map.addLayer(L.mapbox.markerLayer("mixd.map-j47aonim"));
...
function centerMap() {
	if (mapbox.length) {
		if ($(window).width() < 940) {
			pos = "53.998057814183554, -1.539759635925293";
...
function resize_copy() {
	$(window).width() > 940 ? $(".no-ie .js-restrict p").each(function() {
		var e = $(this).parent(),
			t = e.find(".block-heading").outerWidth();
		$(this).width(t)
...
function resize_blog_feature() {
	feature = $(".no-ie .brick--featured .blog-post__inner");
	if (feature.length) if ($(window).width() > 1151) {
		heightNeeded = $(".brick-2 .blog-post").height() + $(".brick-3 .blog-post").height() - $(".brick--featured img").height() - 23;
		feature.height(heightNeeded)
	} else $(window).width() < 480 ? feature.height("7.125em") : feature.height("auto")
}
function lift_page() {
	container = $(".page__body--lift");
	pagebg = $(".page--blog");
	image = $(".single-post__image");
	margin = 16;
	$(window).width() > 480 ? margin = 48 : margin = 16;
	$(window).width() > 940 && (margin = 128);
	if (pagebg.length) {
		container.css("margin-top", -image.height());
		pagebg.css("min-height", image.height() + $(".header").outerHeight() + margin)
	}
}
function pageInView() {
...
function backToTop() {
	var e = $(window).scrollTop(),
		t = $(".back-to-top");
	if (e >= 900 && !anchorExists) {
		t.fadeIn(1e3);
		anchorExists = !0
	} else if (e < 900 && anchorExists) {
		t.fadeOut(1e3);
		anchorExists = !1
	}
}
window.Modernizr = function(e, t, n) {
	...
	for (var P in N) D(N, P) && (O = P.toLowerCase(), c[O] = N[P](), L.push((c[O] ? "" : "no-") + O));
	return c.addTest = function(e, t) {
		if (typeof e == "object") for (var r in e) D(e, r) && c.addTest(r, e[r]);
		else {
			e = e.toLowerCase();
			if (c[e] !== n) return c;
		...
		(function() {
			try {
				...
				e.innerHTML = "<xyz></xyz>", h = "hidden" in e, m = e.childNodes.length == 1 ||
				function() {
					t.createElement("a");
		...
		return u([e])
	}, c.testAllProps = f, c.testStyles = M, c.prefixed = function(e, t, n) {
		return t ? f(e, t, n) : f(e, "pfx")
	}, p.className = p.className.replace(/(^|\s)no-js(\s|$)/, "$1$2") + (h ? " js " + L.join(" ") : ""), c
	...
	return Modernizr.testStyles("#modernizr{font-family:Arial,sans;font-size:300em;}", function(r) {
		t.innerHTML = "&#5987", n.innerHTML = "&#9734", r.appendChild(t), r.appendChild(n), e = "offsetWidth" in t && t.offsetWidth !== n.offsetWidth
	}), e
	...
	e.fn.fitVids = function(t) {
		...
		return this.each(function() {
			...
			r.each(function() {
				...
				var n = this.tagName.toLowerCase() == "object" || t.attr("height") ? t.attr("height") : t.height(),
...
window.Rainbow = function() {
	function e(e) {
		var t, n = e.getAttribute && e.getAttribute("data-language") || 0;
		if (!n) {
			e = e.attributes;
			for (t = 0; t < e.length; ++t) if ("data-language" === e[t].nodeName) return e[t].nodeValue
		}
		return n
	}
	function t(t) {
		var n = e(t) || e(t.parentNode);
		if (!n) {
			var r = /\blang(?:uage)?-(\w+)/;
			(t = t.className.match(r) || t.parentNode.className.match(r)) && (n = t[1])
		}
		return n
	}
	function n(e, t) {
		for (var n in h[v]) {
			n = parseInt(n, 10);
			if (e == n && t == h[v][n] ? 0 : e <= n && t >= h[v][n]) delete h[v][n], delete c[v][n];
			if (e >= n && e < h[v][n] || t > n && t < h[v][n]) return m
		}
		return !1
	}
	function r(e, t) {
		return '<span class="' + e.replace(/\./g, " ") + (w ? " " + w : "") + '">' + t + "</span>"
	}
	function i(e, t, u, f) {
		var l = e.exec(u);
		if (l) {
			++y;
			!t.name && "string" == typeof t.matches[0] && (t.name = t.matches[0], delete t.matches[0]);
			var p = l[0],
				d = l.index,
				m = l[0].length + d,
				g = function() {
					...
					w = function(e, n, i) {
						if (e >= n.length) i(p);
							...
							if (s) {
								var u = t.matches[n[e]],
									f = u.language,
									c = u.name && u.matches ? u.matches : u,
									h = function(t, s, o) {
										var u;
										u = 0;
										var a;
										for (a = 1; a < n[e]; ++a) l[a] && (u += l[a].length);
										s = o ? r(o, s) : s;
										p = p.substr(0, u) + p.substr(u).replace(t, s);
										w(++e, n, i)
									};
								f ? a(s, f, function(e) {
									h(s, e)
								}) : typeof u == "string" ? h(s, s, u) : o(s, c.length ? c : [c], function(e) {
									h(s, e, u.matches ? u.name : 0)
								})
							} else w(++e, n, i)
						}
					};
	...
	function o(e, t, n) {
		function r(t, s) {
			s < t.length ? i(t[s].pattern, t[s], e, function() {
				r(t, ++s)
			}) : u(e, function(e) {
				delete c[v];
				delete h[v];
	...
	function u(e, t) {
		function n(e, t, r, i) {
			if (r < t.length) {
				...
				var s = t[r],
					o = c[v][s],
					e = e.substr(0, s) + e.substr(s).replace(o.replace, o["with"]),
					s = function() {
						n(e, t, ++r, i)
					};
				0 < b % 250 ? s() : setTimeout(s, 0)
			} else i(e)
		}
		var r = s(c[v]);
		n(e, r, 0, t)
	...
	function a(e, t, n) {
		var r = p[t] || [],
			i = p[g] || [],
			t = d[t] ? r : r.concat(i);
		o(e.replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/&(?![\w\#]+;)/g, "&amp;"), t, n)
	}
	function f(e, n, r) {
		if (n < e.length) {
			var i = e[n],
				s = t(i);
			return -1 < (" " + i.className + " ").indexOf(" rainbow ") || !s ? f(e, ++n, r) : (s = s.toLowerCase(), i.className += i.className ? " rainbow" : "rainbow", a(i.innerHTML, s, function(t) {
				i.innerHTML = t;
				c = {};
				h = {};
				E && E(i, s);
	...
	function l(e, t) {
		var e = e && "function" == typeof e.getElementsByTagName ? e : document,
			n = e.getElementsByTagName("pre"),
			r = e.getElementsByTagName("code"),
			i, s = [],
			o = [];
		for (i = 0; i < n.length; ++i) n[i].getElementsByTagName("code").length ? n[i].innerHTML = n[i].innerHTML.replace(/^\s+/, "").replace(/\s+$/, "") : s.push(n[i]);
	...
	return {
		...
		color: function(e, t, n) {
			if ("string" == typeof e) return a(e, t, n);
			if ("function" == typeof e) return l(0, e);
			l(e, t)
		}
	}
}();
...
var svgeezy = function() {
		return {
			...
			fallbacks: function() {
				while (this.imgL--) if (!this.hasClass(this.images[this.imgL], this.avoid) || !this.avoid) {
...
(function(e, t, n) {
	var r = function(e) {
		...
		s = function(e, t) {
			i(e, t) || (e.className = e.className === "" ? t : e.className + " " + t)
		},
		o = function(e, t) {
		...
		a = t.documentElement,
		f = e.Modernizr.prefixed("transform"),
		...
		c = function() {
			...
			return e.hasOwnProperty(l) ? e[l] : !1
		}();
	e.App = function() {
		var n = !1,
			r = {},
			f = t.getElementById("inner-wrap"),
			h = !1,
			p = "js-nav";
		r.init = function() {
			if (n) return;
			n = !0;
			var d = function(e) {
					e && e.target === f && t.removeEventListener(c, d, !1);
					h = !1
				};
			r.closeNav = function() {
				if (h) {
					var n = c && l ? parseFloat(e.getComputedStyle(f, "")[l + "Duration"]) : 0;
					n > 0 ? t.addEventListener(c, d, !1) : d(null)
				}
				o(a, p);
				h = !1
			};
			...
			r.toggleNav = function(e) {
				h && i(a, p) ? r.closeNav() : r.openNav();
				e && e.preventDefault()
			};
			t.getElementById("nav-open-btn").addEventListener("click", r.toggleNav, !1);
			//t.getElementById("nav-close-btn").addEventListener("click", r.toggleNav, !1);
...
$(".js-nav-toggle").click(function(e) {
	$(".nav--primary").slideToggle().toggleClass("nav--open");
	return !1
});
...
var gf_placeholder = function() {
		$(".gform_wrapper .gplaceholder").find("input, textarea").filter(function(e) {
			var t = $(this);
			if (this.nodeName == "INPUT") {
				var n = this.type;
				return n != "hidden" && n != "file" && n != "radio" && n != "checkbox"
			}
			return !0
		}).each(function() {
			var e = $(this),
				...
				n = this.id;
			if (t) {
				var r = $("label[for=" + n + "]").hide(),
					i = r.last().text();
				i.length > 0 && i[i.length - 1] == "*" && (i = i.substring(0, i.length - 1));
				e[0].setAttribute("placeholder", i)
			}
		})
	};
if ($("#disqus_thread").length > 0) {
	var disqus_shortname = "mixdweb";
	(function() {
